...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean isOk = false, isAdmin = false;
        String u = request.getParameter("1");
        String p = request.getParameter("2");
        String r = request.getParameter("3");
        ...
        for (Account acc : list) {
            if (acc.getUsername().equalsIgnoreCase(u) && acc.getPassword().equalsIgnoreCase(p)) {
                x = acc;
                isOk = true;
                if (acc.isAdmin()) {
                    isAdmin = true;
